...
int start_dl(char* sv_file, char* cl_file, char* sv_ip, int sv_port){
    ...
    int sd, ret, block_n, exp_block_n, read_bytes, pkt_type, wr_bytes;
    int tx_started = 0;
    char exists_choice;
    ...
    //exp_block_n contiene il numero di blocchi perche' e' stato incrementato prima di uscire
    printf("Trasferimento completato (%d/%d blocchi).\n", exp_block_n, exp_block_n);
